...
local function initialize_dialog_box_features(game)
  ...
  function dialog_box:show_dialog()
    ...
    local font = dialog.font or default_font
    if font ~= dialog_box.font then
        dialog_box.font = font
        for i = 1, nb_visible_lines do
            dialog_box.line_surfaces[i]:set_font(font)
        end
    end

    local font_size = dialog.font_size or default_font_size
    if font_size ~= dialog_box.font_size then
        dialog_box.font_size = font_size
        for i = 1, nb_visible_lines do
            dialog_box.line_surfaces[i]:set_font_size(font_size)
        end
    end
